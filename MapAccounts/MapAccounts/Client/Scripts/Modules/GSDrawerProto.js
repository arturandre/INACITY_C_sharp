...
GSDrawer.prototype.getImagesFromSelectedStreet = function (callback, interpolate) {
	...
	if (!!that.originalImages && that.originalImages.length > 0 &&
		//Isso garante que numa mesma rua se o nível de detalhamento aumenta (por causa da interpolação que não havia sido selecionada antes)
		//então novas imagens deverão ser selecionadas ao invés de retornar o conjunto em cache.
...
GSDrawer.prototype.setImgPresentationPosition = function (pos) {
	...
	if (!that.imgPreview.onload) {
		that.imgPreview.onload = function (e) {
			console.log("Imagem de index: " + that.imgIndex + " carregada.");
			if (that.autoplay) {
	...
	if (!that.imgPreview.onerror) {
		that.imgPreview.onerror = function (e) {
			console.log("Erro na imagem de index: " + that.imgIndex);
			that.setImgPresentationPosition(that.imgIndex);
			that.imgIndex++;
			that.imgIndex %= that.originalImages.length;
		}
